((win) => {
  ...
  domReady(() => {
    ...
    const articleUrl = encodeURIComponent('https://www.bbc.co.uk/news/world-europe-49922779')
    ...
    asyncForEach(listIfrArr.filter(ifr => !!ifr.getAttribute('data-src')), async (ifr, idx) => {
      ...
      if (projectId) {
        const { data: { podcast_id } } = await fetch(`http://spkt.local:3000/s/${projectId}/${articleUrl}`, fetchOptions)
          .then(response => (
        ...
        if (podcast_id) {
          const resSrc = await makeSrc(`http://spkt.local:3000/a/${podcast_id}`)
